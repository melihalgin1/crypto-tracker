...
const CoinDetail = ({ coinId, currency, onBack, lang = 'en' }) => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        if (!json.prices || json.prices.length === 0) throw new Error("No data available.");

        const formattedData = json.prices
          .filter(item => item[1] !== null)
          .map(([timestamp, price]) => ({ date: timestamp, price: price }));
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          ...
          <div className="flex items-center gap-2">
            <p className="text-sm text-gray-500">{t.priceHistory} ({currency.toUpperCase()})</p>
            <span className={`text-xs font-bold px-2 py-0.5 rounded ${isProfit ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              {days === 1 ? '24H' : days} {isProfit ? t.trendUp : t.trendDown}
            </span>
          </div>
        </div>
        
        {/* FIX: Darker background (bg-gray-200) makes white buttons pop */}
      ...
      <div ref={containerRef} className="bg-white border rounded-2xl shadow-sm p-4 h-[450px] relative w-full">
        {status === 'loading' && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-white z-20">
            <div className="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin mb-2"></div>
            <p className="text-gray-400 text-sm font-medium">{t.chartLoading}</p>
          </div>
        )}

        {status === 'error' && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-white z-20 text-center px-4">
            <p className="text-red-500 font-bold mb-1">{t.chartError}</p>
            <p className="text-gray-500 text-sm mb-4">{errorMessage}</p>
            <button onClick={() => setDays(days)} className="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded text-sm font-medium">{t.retry}</button>
          </div>
        )}

        {status === 'success' && chartWidth > 0 && (
           <div className="w-full h-full flex items-center justify-center">
              <AreaChart width={chartWidth - 32} height={400} data={data}>
                <defs>
                ...
                <Tooltip 
                  ...
                  formatter={(value) => [`${symbol}${value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 10 })}`, 'Price']}
                />
                <Area type="monotone" dataKey="price" stroke={color} fillOpacity={1} fill={"url(#colorPrice-" + coinId + ")"} strokeWidth={2} animationDuration={500} />
              </AreaChart>
          </div>
        )}
      </div>
